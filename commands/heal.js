...
module.exports = {
    ...
    execute(message, args){
        ...
        let sql = `SELECT U.hp, U.max_hp, I.healthpotion FROM Users U, Inventory I WHERE I.id = U.id AND U.id = '${message.author.id}'`;
        connection.query(sql, (err, rows) =>{
            ...
            // otherwise user can heal
                ...
                let sql3 = `UPDATE Inventory SET healthpotion = healthpotion -1 WHERE id = '${message.author.id}'`;
                // query the db
                connection.query(sql2);
                connection.query(sql3);

                // print message to user
                message.channel.send(`${message.author}, Your hp has been fully healed by drinking a ${consumables['healthpotion']['emoji']}`);
                return;
